...
// Make Table every 60s gi jun
#define MAKE_ADDRESSES_INTERVAL 10 
...
void CConnman::DumpAddresses()
{
    ...
    CAddrDB adb;
    adb.Write(addrman);
    //addrman.writetable(nStart, addrman); //gi jun

    LogPrint(BCLog::NET, "Flushed %d addresses to peers.dat  %dms\n",
           addrman.size(), GetTimeMillis() - nStart);
}

void CConnman::MakeAddresses() //gi jun
{
    int64_t nStart = GetTimeMillis();
    addrman.writetable(nStart, addrman);
}
...
bool CConnman::Start(CScheduler& scheduler, const Options& connOptions)
{
    ...
    {
        CAddrDB adb;
        if (adb.Read(addrman)) //point of reset, reformating CAddrMan addrman : gi jun
    ...
    //Make TXT File gi jun
    scheduler.scheduleEvery(std::bind(&CConnman::MakeAddresses, this), MAKE_ADDRESSES_INTERVAL * 1000);
